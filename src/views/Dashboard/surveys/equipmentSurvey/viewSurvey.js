...
import {
    surveyAgentInfo,
    surveyCropInfo, surveyEquipmentInfo,
...
function ViewEquipmentSurvey(props) {
    const {classes} = props;
    const {main, container, dataBox, labelBox, inputBox, divider, containerTitle} = classes;
    const [survey, setSurvey] = useState('');

    const allSurveys = [
        {title: "Farmer Details", survey: surveyFarmerInfo, prefix: ''},
        {title: "Equipment Details", survey: surveyEquipmentInfo, prefix: ''},
    ...
    const getSurvey = (id) => {
        FirebaseDataService.getSurvey(id, 'equipment')
            .then((data) => {
                setSurvey(data.data())
                console.log(survey)
            })
...
export default withStyles(useStyles, {withTheme: true})(ViewEquipmentSurvey);